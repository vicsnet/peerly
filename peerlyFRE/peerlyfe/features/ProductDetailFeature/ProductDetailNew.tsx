...
export function ProductDetailNew({
  ...
  const { id, query, vendorName } = router.query;
  ...
  // To extract "HemmaH_OSStore" from the URL:
  // If using Next.js router, vendorName will be available in router.query.vendorName
  // Example: /products/[id]/[productId]?vendorName=HemmaH_OSStore
  // So, you can access it as:
  // const vendorName = router.query.vendorName as string | undefined;

  const {addToCart, items, removeFromCart} = useCartState()
  ...
  const { data: vendorProduct } = useQuery({
    ...
    queryFn: async (): Promise<any> => {
      ...
      const vendorData = await shopContract.getProduct(query);
      const res = await fetch(vendorData[7]);
      const specification = await res?.json();


      return { image: vendorData[0], description: vendorData[1], name: vendorData[2], price: Number(vendorData[3]), totalStock: Number(vendorData[4]), sold: Number(vendorData[5]), sizes: vendorData[6], specification: specification };
    },
    enabled: !!id,
  });
  ...
  const incrementQuantity = () => {
    if (vendorProduct && quantity < vendorProduct.totalStock) {
      setQuantity(quantity + 1);
    } else {
  ...
  const handlePrevImage = () => {
    if (vendorProduct && vendorProduct.image.length > 0) {
      setCurrentImageIndex((prev) =>
        prev === 0 ? vendorProduct.image.length - 1 : prev - 1
      );
    }
  };
  ...
  const handleNextImage = () => {
    if (vendorProduct && vendorProduct.image.length > 0) {
      setCurrentImageIndex((prev) =>
        prev === vendorProduct.image.length - 1 ? 0 : prev + 1
      );
    }
  };
  ...
  const getTabContent = () => {
    if (activeTab === 'details') {
      const description = vendorProduct?.description || '';
      ...
      const sizes = vendorProduct?.sizes?.map(([size, quantity]: [any, any]) => ({
        size,
        quantity: Number(quantity) // convert BigInt to number if needed
      })) || [];

      const baseSpecs = [
        `SKU: ${vendorProduct?.specification.SKU}`,
        // `Product Line: ${vendorProduct?.productLine || 'Liangzai2021'}`,
        `ProductType: ${vendorProduct?.specification.product_type}`,
        `Made In: ${vendorProduct?.specification.production_country || 'China'}`,
        `Sizes: ${sizes.map((s: any) => s.size)}`,

        `Weight (kg): ${vendorProduct?.specification.Weight}`,
        // `Color: ${vendorProduct?.color || 'White'}`,
        // `Main Material: ${vendorProduct?.material || 'N/A'}`
      ];

      const additionalSpecs = vendorProduct?.specification.additionalSpecs
...
console.log('clicked');
    ...
    // Check if there are any items in the cart from a different vendor than the current product
    const hasItemsFromOtherVendor = items.some(
      (item) => item.vendorId !== (typeof id === "string" ? id : Array.isArray(id) ? id[0] : "")
    );
  ...
  const addProductToCart = () => {
    ...
    addToCart({
      ...
      image: vendorProduct.image[0],
      vendorId: typeof id === "string" ? id : Array.isArray(id) ? id[0] : "",
      quantity: quantity,
      vendorName: typeof vendorName === "string" ? vendorName : Array.isArray(vendorName) ? vendorName[0] : ""
    });
    console.log(`item added`)

    toast.success(`${vendorProduct.name} added to cart`);
  };
  ...
  const handleBuyNowClick = () => {
    ...
    const hasItemsFromOtherVendor = items.some(item => item.vendorId !== vendorProduct.vendorId);
  ...
  const handleClearCartAndBuyNow = () => {
        ...
        const buyNowProductData = {
          id: vendorProduct.id,
          name: vendorProduct.name,
          price: vendorProduct?.discountPrice || vendorProduct.price,
          image: vendorProduct.images[0],
          quantity: quantity,
          vendorId: vendorProduct.vendorId,
        //   vendorName: vendorName
        };
  ...
  if (vendorProduct && vendorProduct.image === "" && vendorProduct.name === "" && vendorProduct.description === "") {
    return (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
        ...
        <div className="flex flex-row gap-6 w-full">
          ...
          <div className="flex flex-col gap-6 w-[796px]">
            ...
            <div className="bg-slate-100 rounded-md w-fit">
              <BackButton onClick={() => router.back()} label="Back to Products" />
            </div>
              ...
              <div className="flex flex-row gap-4 p-0">
                ...
                <div className="flex flex-col gap-2 shrink-0">
                  ...
                  <div className="relative w-[328px] h-[328px] rounded-xl overflow-hidden bg-slate-50">
                    <ImageWithFallback
                      src={vendorProduct?.image[currentImageIndex]}
                  ...
                  <div className="flex gap-2 overflow-x-auto">
                    {vendorProduct?.image.map((image: string, index: number) => (
                      ...
                      >
                        <ImageWithFallback
                          src={image}
                          alt={`${vendorProduct.name} thumbnail ${index + 1}`}
                          className="w-full h-full object-cover"
                        />
                      </button>
                    ))}
                ...
                <div className="flex-1 h-96 flex flex-col gap-3 p-0 overflow-hidden">
                  ...
                  <div className="bg-slate-100 rounded-md p-1 flex w-fit">
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm font-medium transition-colors ${activeTab === 'details'
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded text-sm font-medium transition-colors ${activeTab === 'specification'
                          ? 'bg-white text-slate-900 shadow-sm'
                          : 'text-slate-700 hover:text-slate-900'
                        }`}
                    >
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex gap-6">
                ...
                <div key={"id"} className="w-[322px] rounded-xl overflow-hidden">
      ...
      <VendorMismatchModal 
        ...
        currentVendorName={vendorProduct?.name || "the current vendor"}
      />
    </div>
  );
}
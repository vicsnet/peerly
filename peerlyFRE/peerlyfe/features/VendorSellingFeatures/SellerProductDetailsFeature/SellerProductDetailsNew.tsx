...
import { EditProductModalNew } from "./components/EditProductModalNew";
import { useQuery } from "@tanstack/react-query";
import { useShop } from "@/contexts/zustand/ZustandState";
import { DeleteProductModalNew } from "./components/DeleteProductModalNew";
...
// Add type for specification data
...
export function SellerProductDetailsNew({
    product,
    onBack,
    onEdit,
    onDelete,
    ...
    console.log('productDetailsNew', product);
    ...
    const images = product.productImage?.length
      ...
      const {data: specification} = useQuery<SpecificationData>({
        queryKey:['productSpecification', product.specification],
        queryFn: async () => {
            const res = await fetch(product.specification);
            return res.json();
        },
        enabled: !!product.specification 
      })   
    
    console.log('specification', specification);
    
    // Determine if product is active/published based on either the active flag or inventory
    const isProductActive = Number(product.totalStock) > 0;
    ...
    //     if (quantity < Number(product.totalStock)) {
    //         setQuantity(quantity + 1);
    //     }
    // };
    ...
    //     onDelete(Number(product.productId));
                        ...
                        <div className="box-border content-stretch flex flex-row gap-3 items-start justify-start p-0 relative">
                            ...
                            {/* <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-48">
                                    <DropdownMenuItem onClick={() => toast.info(`Sharing product: ${product.productName}`)} className="cursor-pointer">
                                        Share Product
                                    </DropdownMenuItem>
                                    <DropdownMenuItem onClick={() => onToggleStatus(Number(product.productId), !isProductActive)} className="cursor-pointer">
                        ...
                        <div className="box-border content-stretch flex flex-col gap-6 items-start justify-start p-0 relative size-full">
                            <div className="relative shrink-0 w-full">
                                <div className="flex-wrap box-border content-start flex gap-2 items-start justify-start p-0 relative w-full">
                                    {(Array.isArray(images) ? images : [images]).map((image: string, index: number) => (
                                        <div key={index} className="bg-cover bg-center bg-no-repeat rounded-lg shrink-0 size-60">
                                            <ImageWithFallback
                                                src={image}
                                        ...
                                        {specification && (
                                            <div className="space-y-2">
                                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                                    <span className="text-[14px] font-normal text-gray-600">SKU</span>
                                                    <span className="text-[14px] font-medium text-gray-900">{specification.SKU}</span>
                                                </div>
                                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                                    <span className="text-[14px] font-normal text-gray-600">Weight</span>
                                                    <span className="text-[14px] font-medium text-gray-900">{specification.Weight}</span>
                                                </div>
                                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                                    <span className="text-[14px] font-normal text-gray-600">Product Type</span>
                                                    <span className="text-[14px] font-medium text-gray-900">{specification.product_type}</span>
                                                </div>
                                                <div className="flex justify-between items-center py-2 border-b border-slate-100">
                                                    <span className="text-[14px] font-normal text-gray-600">Production Country</span>
                                                    <span className="text-[14px] font-medium text-gray-900">{specification.production_country}</span>
                                                </div>
                                                {specification.additionalSpecs && specification.additionalSpecs.length > 0 && (
                                                    <div className="flex justify-between items-start py-2">
                                                        <span className="text-[14px] font-normal text-gray-600">Additional Specs</span>
                                                        <div className="text-[14px] font-medium text-gray-900">
                                                            {specification?.additionalSpecs.map((spec: string, index: number) => (
                                                                    ...
                                                                    <div className="[grid-area:1_/_1] ml-0 mt-0 relative">
                                                                        <div className="box-border content-stretch flex flex-row gap-1 items-center justify-start p-0 relative">
                                                                            <div className={`text-[14px] font-medium leading-[14px] ${isProductActive ? 'text-green-900' : 'text-amber-900'} relative shrink-0 text-nowrap`}>
                                                                                <p className="block leading-[14px] whitespace-pre">
                                                                                    {isProductActive ? "Published" : "No stock"}
                                                                                </p>
                                                                            </div>
                                                                        </div>
                                                                    </div>
